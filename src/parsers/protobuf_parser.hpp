...
public:
	...
	idx_t ParseTracesToTypedRows(google::protobuf::io::ZeroCopyInputStream &stream, vector<vector<Value>> &rows);
	idx_t ParseLogsToTypedRows(const char *data, size_t length, vector<vector<Value>> &rows);
	idx_t ParseLogsToTypedRows(google::protobuf::io::ZeroCopyInputStream &stream, vector<vector<Value>> &rows);
	idx_t ParseMetricsToTypedRows(const char *data, size_t length, vector<vector<Value>> &rows);
	idx_t ParseMetricsToTypedRows(google::protobuf::io::ZeroCopyInputStream &stream, vector<vector<Value>> &rows);
