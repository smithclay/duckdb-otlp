...
- Live ingestion is exposed by two scheme-bound serve functions (no URI mixing — each rejects the other's scheme): **`otlp_serve`** on the `otlp:` scheme (default port 4318) and **`otap_serve`** on the `otap:` scheme (default port 4317). `otlp_stop` / `otlp_flush` / `otlp_server_list` are transport-agnostic — they dispatch by the scheme-aware canonical listen URI, so `otap:host:4317` and `otlp:host:4318` are distinct servers. The transport is **not** encoded in the scheme: `otlp_serve` picks it via the `transport := 'http'|'grpc'` named param (default `http`); `otap_serve` is always gRPC.
  ...
  - **One name per setting, one error per run**: storage configuration used to spell the same concept differently per mode (five names for an R2 access key, `NEON_PG*` vs `PG*` for the same Postgres catalog, six for a bucket) — about 70 variable names for ~25 concepts. Each mode now reads one canonical name: the cloud's own convention where one exists (`AWS_*`, `PG*` libpq names), `R2_*` for R2, `S3_*` for S3, `DUCKDB_OTLP_*` only for this daemon's own settings. `MissingSettings` (`server_config.cpp`) accumulates every missing setting for a mode and `ConfigureMode` throws once naming all of them, deduplicated by variable — configuring `r2-data-catalog` took six runs to satisfy when each error named a single variable. `MODES` is one table of `{name, configure}` so the supported-mode list in the error cannot drift from the set actually handled.
  - **`parquet` mode is readable by `query`/`export`**: it has no catalog (`config.catalog` is empty; the seal COPYs to `<root>/<table>/year=/month=/day=/`), so both commands dead-ended — `FROM otlp_logs` raised "Table with name otlp_logs does not exist", and `export` reported an empty catalog and advised running `serve`, a state the mode can never reach. `RegisterParquetExportViews` (`commands.cpp`, called from `OpenConfiguredDatabase` after init SQL) defines one view per signal over the dataset. It reads with `hive_partitioning = false` — the seal sets `WRITE_PARTITION_COLUMNS false`, so year/month/day exist only in the path, and letting read_parquet infer them appended three columns that no catalog mode's export has (verified: the exported header is now byte-identical between `local-ducklake` and `parquet`). Only signals with files get a view, so `--signal all` still skips cleanly; `ExistingSignalTables` unions `duckdb_views()` so the skip logic sees them. A read-only database cannot hold a view, so that path falls back to session-scoped `TEMP VIEW`s.
