...
Two constraints shape what can be written there. `duckdb_functions()` picks which description belongs to which overload by matching `parameter_types` against the overload's **positional** arguments, so a set with several overloads needs one description each (`otlp_stop()` and `otlp_stop(uri)` say different things). And as of DuckDB 2.0 a `FunctionDescription`'s `parameter_names` are only a *fallback*: `duckdb_functions()` takes the names off the function's own signature whenever their count matches the rendered parameter list, and falls back to the description only when it does not (`GetParameterNames` in `duckdb_functions.cpp`; in v1.5.5 the description won outright). A `ScalarFunction` can name its parameters — build it from a `FunctionSignature` of named `FunctionParameter`s, as `otlp_uri_parser` does. A `TableFunction` cannot: it is constructed from a bare `vector<LogicalType>` and its extractor emits `col0`, `col1`, … unconditionally, so every `read_ot*` reader and every control-plane table function reports `colN` no matter what its description says. Their `parameter_names` are kept anyway, to document the argument and to take effect if upstream restores the override. `otlp_serve`/`otap_serve` leave `parameter_names` empty for a second reason that outlives this: the named parameters' order comes from an `unordered_map` the catalog copies, so it cannot be reproduced at registration time.
...
- Live ingestion is exposed by two scheme-bound serve functions (no URI mixing — each rejects the other's scheme): **`otlp_serve`** on the `otlp:` scheme (default port 4318) and **`otap_serve`** on the `otap:` scheme (default port 4317). `otlp_stop` / `otlp_flush` / `otlp_server_list` are transport-agnostic — they dispatch by the scheme-aware canonical listen URI, so `otap:host:4317` and `otlp:host:4318` are distinct servers. The transport is **not** encoded in the scheme: `otlp_serve` picks it via the `transport := 'http'|'grpc'` named param (default `http`); `otap_serve` is always gRPC.
  ...
  - **Custom DuckDB setup (`--init-sql` / `DUCKDB_OTLP_INIT_SQL`)**: a SQL script run after mode setup and before ingest starts — the escape hatch for DuckDB configuration the modes do not model (extra `ATTACH`, `SET`, secrets, views). Accepted by `serve`, `validate`, `export`, and `query` (the `INIT_SQL_CMDS` mask in `cli.cpp`); **not** by `doctor`, because the container `HEALTHCHECK` runs it on every probe and a liveness check must not execute operator SQL, nor by `convert`, which opens no catalog. Resolved to *contents* in `ServerConfig::FromEnv` (via `ReadSqlFile`) rather than kept as a path, so `validate` both rejects an unreadable script and prints it inside the generated boot SQL; `doctor` never calls `FromEnv`, so the probe never reads it. Failures are fatal. It runs before the default-catalog switch, so a catalog the script attaches can still be named by `--catalog`.
