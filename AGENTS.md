...
- Live OTLP ingestion is supported over **HTTP** (`otlp_serve` / `otlp_stop` / `otlp_server_list` / `otlp_flush`), not gRPC. The HTTP server (`src/otlp_server.cpp`) accepts OTLP/JSON, OTLP/NDJSON, and OTLP/protobuf POSTs to `/v1/logs`, `/v1/traces`, `/v1/metrics`.
  ...
  - **Durability**: the default `buffer := 'memory'` behavior is unchanged: a POST returns **`202 Accepted`** (`{"status":"buffered",...}`) once rows are parsed and buffered in memory, but not yet durable; they commit at the next seal. **`otlp_stop` and `otlp_flush` seal remaining rows before returning; a plain database/connection close does NOT** — buffered-but-un-sealed memory rows can be lost. Opt in to `buffer := 'disk'` with `disk_buffer_dir := '<dir>'` to journal each accepted raw request body to a per-request-kind disk buffer and fsync it before returning 202. Disk mode replays uncheckpointed records on restart for at-least-once recovery. A crash after DuckLake COMMIT but before disk-buffer checkpoint fsync can replay already-committed records, so duplicates are expected in that window. Backpressure: request admission over `max_buffered_bytes` (default 512 MiB) → **`503`**; disk buffer capacity over `disk_max_bytes` per request kind (default 1 GiB) → **`507`**.
  - **Disk buffer config**: `otlp_serve(..., buffer := 'disk', disk_buffer_dir := '<dir>', disk_segment_bytes := 67108864, disk_max_bytes := 1073741824)`. The disk buffer is partitioned into `logs/`, `traces/`, and `metrics/`; a metrics request is journaled/checkpointed once even when it fans out to multiple metric tables. `manifest.json` binds the directory to the configured catalog/schema and startup refuses mismatches.
  - **`otlp_flush(uri, checkpoint := false)`** forces a synchronous seal; `checkpoint := true` also compacts DuckLake (`ducklake_merge_adjacent_files` + `CHECKPOINT`). `otlp_server_list` exposes buffer/seal metrics (`buffered_rows`, `buffered_bytes`, `last_seal_age_ms`, `seals_total`, `seal_last_error`, `catalog_name`) plus disk-buffer metrics (`buffer_mode`, `disk_pending_records`, `disk_buffered_bytes`, `journal_writes_total`, `disk_healthy`, `disk_last_error`, etc.). Verify the ingest/seal path with `test/manual/otlp_serve_concurrency.py` (set `OTLP_DUCKLAKE_DIR` for the DuckLake path; pass `--disk` to include disk-buffer scenarios).
