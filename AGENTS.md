...
- Live OTLP ingestion is supported over **HTTP** (`otlp_serve` / `otlp_stop` / `otlp_server_list` / `otlp_flush`), not gRPC. The HTTP server (`src/otlp_server.cpp`) accepts OTLP/JSON, OTLP/NDJSON, and OTLP/protobuf POSTs to `/v1/logs`, `/v1/traces`, `/v1/metrics`.
  ...
  - **Durability**: a POST returns **`202 Accepted`** (`{"status":"buffered",...}`) — rows are buffered, NOT yet durable; they commit at the next seal (within `seal_max_age_ms`, or immediately on `otlp_flush`). A crash loses buffered-but-un-sealed rows (at-most-once for that window; a durable raw-spool journal is a future enhancement). A graceful `otlp_stop` / database close seals remaining rows first (no loss on graceful shutdown). Backpressure: total buffered bytes over `max_buffered_bytes` (default 512 MiB) → **`503`**.
