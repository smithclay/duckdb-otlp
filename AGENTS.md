...
### Attribute bags are JSON text; VARIANT is one opt-in flag on one struct

The Rust backend has no VARIANT encoder — it writes every `*_attributes` bag as a JSON object into a `Utf8` column (`src/schema/arrow.rs`, `src/batch/view_json.rs`), and that is still what crosses the FFI with `attributes_as_variant := true`. The flag only moves where the text stops being text: `OtlpArrowSchemaOptions::attributes_as_variant` maps those columns to `LogicalType::VARIANT()` in `GetArrowSchemaColumns`, and `CopyArrowToDuckDB` sees a VARIANT output vector, copies the Arrow strings into a `LogicalType::JSON()` vector and runs DuckDB's JSON→VARIANT cast once per chunk. That cast is a *default* cast over a VARCHAR alias (`ConvertJSONToVariant`, core, yyjson is in core), so the path needs no `ClientContext` and no `json` extension, and it works identically on WASM.

One flag on the options struct is what makes the read scan and the ingest server share the whole implementation: both already route through `GetArrowSchemaColumns` + `CopyProjectedArrowStructToDataChunk`, so neither the scan nor the seal has a VARIANT branch of its own. The columns are chosen by the `_attributes` suffix (`OtlpIsAttributeBagColumn`) rather than a second list to keep in sync with the Rust schemas; `dropped_attributes_count` is a counter and does not match, and the JSON *array* columns (`events_json`, `links_json`, `exemplars_json`) and `body` are deliberately out of scope — their names say `json`, and renaming them is a breaking change this flag does not need to make.

For the server it is part of the destination table's shape, not a runtime preference: `CreateOrValidateTable` compares column types exactly, so a table created on the other setting is a hard startup error. That error names the knob and the `ALTER TABLE ... SET DATA TYPE ... USING` that migrates it, and the `USING` is load-bearing — a bare `VARCHAR`→`VARIANT` cast stores the whole JSON document as a VARIANT *string* instead of parsing it, and `VARIANT`→`VARCHAR` renders DuckDB's display form rather than JSON. Attribute promotion follows the bag's type: `CAST(variant_extract(bag, 'k') AS VARCHAR)` instead of `json_extract_string(bag, '$."k"')`. `variant_extract` takes a literal key, not a path, which is what a dotted OTLP key like `service.name` needs.

What this is *not* is an Arrow-native VARIANT. DuckDB registers no `arrow.parquet.variant` extension type (`arrow_type_extension.cpp` has uuid/bool8/json) and refuses to export VARIANT over Arrow at all, so the C Data Interface cannot carry one. The way to remove the JSON round trip is for the crate to emit the Parquet-variant binary encoding (metadata‖value) as an Arrow `Binary` column and convert it with the parquet extension's `variant_bytes_to_variant(BLOB)`; that is a separate, breaking change to otlp2records. Requires DuckDB ≥ 1.5 (where VARIANT landed in core) and, for a lakehouse target, DuckLake ≥ 0.4. Measured on the pinned DuckDB, the flag costs ~6% ingest throughput and ~40% more wall time per seal and saves ~40-49% on disk. Query speed is version-dependent and the gap is large: on 1.5 a bag extraction is several times slower than `json_extract_string` (1.5 shreds on write but reconstructs the whole VARIANT per row on read), while on 2.0 -- built and measured against `v2.0-cyanoptera` -- the same extraction is ~9-17x *faster* than JSON on string keys and GROUP BY, because shredded execution reads the one subcolumn instead. Numeric extraction is still ~2x behind JSON there. That is why the flag is opt-in and off, and why the docs state the payoff per version rather than promising a scan win. On the daemon this and attribute promotion are `INGEST_SHAPE_CMDS` flags (`--attributes-as-variant`, `--promote-resource-attributes`, `--promote-scope-attributes`): `serve` and `validate` only, because `doctor` never calls `ServerConfig::FromEnv` and creates no table, so accepting them there would be the silent no-op the per-command flag table exists to prevent.
