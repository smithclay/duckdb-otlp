...
- Live OTLP ingestion is supported over **HTTP** (`otlp_serve` / `otlp_stop` / `otlp_server_list` / `otlp_flush`), not gRPC. The HTTP server (`src/otlp_server.cpp`) accepts OTLP/JSON, OTLP/NDJSON, and OTLP/protobuf POSTs to `/v1/logs`, `/v1/traces`, `/v1/metrics`.
  ...
  - **Durability**: a POST returns **`202 Accepted`** (`{"status":"buffered",...}`) — rows are buffered, NOT yet durable; they commit at the next seal (within `seal_max_age_ms`, or immediately on `otlp_flush`). **`otlp_stop` and `otlp_flush` seal remaining rows before returning; a plain database/connection close does NOT** — buffered-but-un-sealed rows are lost (the shutdown seal runs after the DuckDB instance is torn down, when it can no longer write). So to guarantee no loss, call `otlp_flush` or `otlp_stop` before closing the database. A crash likewise loses the un-sealed window (at-most-once; a durable raw-spool journal / earlier shutdown hook is a tracked follow-up). Backpressure: total buffered bytes over `max_buffered_bytes` (default 512 MiB) → **`503`**.
  - **`otlp_flush(uri, checkpoint := false)`** forces a synchronous seal; `checkpoint := true` also compacts DuckLake (`ducklake_merge_adjacent_files` + `CHECKPOINT`). `otlp_server_list` exposes buffer/seal metrics (`buffered_rows`, `buffered_bytes`, `last_seal_age_ms`, `seals_total`, `seal_last_error`, `catalog_name`). Verify the ingest/seal path with `test/manual/otlp_serve_concurrency.py` (set `OTLP_DUCKLAKE_DIR` for the DuckLake path).
