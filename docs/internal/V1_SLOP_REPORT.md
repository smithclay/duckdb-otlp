...
- API-004/SLOP-001 (delete dead `defs.rs`), SLOP-002/FFI-005 (drop never-produced FFI status codes), FFI-002 (remove generated header + cbindgen), FFI-001 (export_record_batch write-order note) — **fixed**, no schema-name change; 116 cargo tests pass. Opened as **otlp2records PR #17** (`smithclay/otlp2records`, branch `cleanup/remove-dead-defs-and-ffi-surface`, off `main`/v0.8.4). The duckdb-otlp submodule pointer (parent commit **241b3fd**) currently references the content-identical local commit **bfeeb36**; `make test` 250 + manual harness PASS. **Release step (user): merge PR #17, tag an otlp2records release, then re-point the submodule at the released SHA.**
