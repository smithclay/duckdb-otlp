...
- API-004/SLOP-001 (delete dead `defs.rs`), SLOP-002/FFI-005 (drop never-produced FFI status codes), FFI-002 (remove generated header + cbindgen), FFI-001 (export_record_batch write-order note) — **fixed and released**, no schema-name change; 116 cargo tests pass. Landed via **otlp2records PR #17** (merged), released as **otlp2records v0.9.0** (minor bump — breaking: dropped public schema API + renumbered FFI status enum). The duckdb-otlp submodule is now pinned to the **v0.9.0** commit `509b0b0` and `.gitmodules` `branch = v0.9.0`; `make test` 250 + schema re-verified OTAP-canonical. Durable (fetchable from the remote).
