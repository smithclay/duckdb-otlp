...
This guide shows you how to run the `duckdb-otlp` Docker image in `s3-tables` mode and stream OTLP/HTTP exports into an Iceberg catalog hosted by **Amazon S3 Tables**.
...
- If startup cannot find AWS credentials, confirm `AWS_PROFILE` in `s3tables.env` matches a profile in the mounted `$HOME/.aws` directory.
- If attachment fails, confirm `S3_TABLES_BUCKET_ARN` is the table bucket ARN from CloudFormation, not an `s3://` path.
