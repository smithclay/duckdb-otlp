...
This guide shows you how to run the `duckdb-otlp` Docker image in `r2-data-catalog` mode and stream OTLP/HTTP exports into an Iceberg catalog hosted by **Cloudflare R2 Data Catalog**.
...
- If startup cannot attach the catalog, confirm `CLOUDFLARE_CATALOG_URI`, `CLOUDFLARE_CATALOG_TOKEN`, `CLOUDFLARE_ACCOUNT_ID`, and `CLOUDFLARE_R2_BUCKET` all refer to the same bucket.
- If startup attaches the catalog but cannot write files, confirm `CLOUDFLARE_ACCESS_KEY_ID` and `CLOUDFLARE_SECRET_ACCESS_KEY` can write objects to the R2 bucket.
...
- R2 Data Catalog currently does not support R2 buckets in a non-default jurisdiction.
...
- If catalog checkpointing is reported as unsupported, no action is required; ingest, flush, and stop durability behavior is unchanged.
